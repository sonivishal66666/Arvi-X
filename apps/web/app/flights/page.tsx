...
import { Search, Star, Clock, ArrowRight, SlidersHorizontal, Plane, Loader2 } from 'lucide-react';
    ...
    <div className="min-h-screen pt-24 pb-16">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div className="text-center mb-10" {...fadeInUp}>
          <Badge className="mb-4 px-4 py-1.5 text-sm rounded-full glass">✈️ Flight Booking</Badge>
          <h1 className="text-4xl md:text-5xl font-bold mb-3 premium-gradient-text">Book Flights</h1>
          <p className="text-muted-foreground max-w-xl mx-auto">Domestic and international flights at the best prices. Compare airlines and book with ease.</p>
        </motion.div>
        ...
        <motion.div className="flex flex-col md:flex-row gap-4 mb-8" {...fadeInUp}>
          <div className="relative flex-1">
            ...
            <Input value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Search flights, airports..." className="pl-10 h-12 glass border-white/10" />
          </div>
          <Button variant="outline" className="h-12 glass-button gap-2" onClick={() => setShowFilters(!showFilters)}>
            <SlidersHorizontal className="w-4 h-4" /> Airlines {selectedAirlines.length > 0 && <Badge>{selectedAirlines.length}</Badge>}
          </Button>
              ...
              <div className="glass p-6 rounded-2xl space-y-6">
                <div>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={() => setSelectedAirlines([])} className="text-xs text-muted-foreground hover:text-foreground">Clear</button>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {airlines.map(a => (
                      <button key={a} onClick={() => setSelectedAirlines(prev => prev.includes(a) ? prev.filter(x => x !== a) : [...prev, a])}
                        className={`px-4 py-2 rounded-xl text-sm transition-all ${
                          selectedAirlines.includes(a) ? 'bg-primary/20 text-primary border border-primary/30' : 'glass-button border border-white/5'
                        }`}>{a}</button>
                    ))}
                  </div>
                </div>
        ...
        {isLoading ? (
          ...
          <motion.div className="text-center py-20" {...fadeInUp}>
            <div className="w-20 h-20 rounded-full glass flex items-center justify-center mx-auto mb-6"><Plane className="w-8 h-8 text-muted-foreground" /></div>
            <h3 className="text-xl font-medium mb-2">No flights found</h3>
            <p className="text-muted-foreground mb-6">Try different dates or destinations</p>
            <Button onClick={() => { setSearchTerm(''); setSelectedAirlines([]); setMinPrice(''); setMaxPrice(''); }}>Clear Filters</Button>
          </motion.div>
        ) : (
                ...
                <Link href={`/services/${service.id}`} className="block group">
                  <div className="glass rounded-2xl overflow-hidden card-hover">
                    <div className="aspect-[16/9] relative overflow-hidden">
                      <img src={service.images?.[0] || flightImages[i % flightImages.length]} alt={service.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
                      <div className="absolute top-4 left-4 flex gap-2">
                        <Badge className="glass">FLIGHT</Badge>
                        {service.discountPercent > 0 && <Badge className="bg-green-500/80 text-white">-{service.discountPercent}%</Badge>}
                      </div>
                      ...
                      <div className="absolute bottom-4 left-4 right-4">
                        ...
                        <p className="text-white/70 text-sm">{service.flight?.airline || 'Premium Airlines'} &middot; {service.flight?.flightNumber || ''}</p>
                      </div>
                    </div>
                    <div className="p-5 space-y-3">
                      ...
                      <div className="flex items-center justify-between text-sm">
                        <div className="flex items-center gap-1.5 text-muted-foreground"><Clock className="w-3.5 h-3.5" /><span>{service.flight?.airline || 'Non-stop'}</span></div>
                        <div className="flex items-center gap-1"><Star className="w-3.5 h-3.5 fill-amber-500 text-amber-500" /><span className="font-medium">{service.rating || '4.6'}</span></div>
                      </div>
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-xs text-muted-foreground line-through">{service.discountPercent > 0 ? formatCurrency(service.basePrice * 1.25) : ''}&nbsp;</p>
                          <p className="text-lg font-bold premium-gradient-text">{formatCurrency(service.basePrice)}</p>
                          <p className="text-xs text-muted-foreground">per person</p>
                        </div>
                        <Button className="rounded-xl group-hover:shadow-lg group-hover:shadow-purple-500/25 transition-all">
                          Book <ArrowRight className="w-3.5 h-3.5 ml-1.5 group-hover:translate-x-1 transition-transform" />
                        </Button>
                      </div>
                    </div>
                  </div>
